...
//		Test.assertEqual(3,dc.calls.size());
	...
	(:test)
	function success_calculateActiveCalories_zero_at_midnight(logger){
		var uiH = new helper();

		var midnight = new Time.Gregorian.Info();
		midnight.year = 2020;
		midnight.hour = 0;
		midnight.min = 0;

		var profile = new UserProfile.Profile();
		profile.gender = UserProfile.GENDER_MALE;
		profile.weight = 75000;
		profile.height = 178;
		profile.birthYear = 1979;

		var calories = 1000;

		Test.assertEqual(0, uiH.calculateActiveCalories(midnight, profile, calories));

		return true;
	}

	(:test)
	function success_calculateActiveCalories_zero_at_midnight_when_high_calories(logger){
		var uiH = new helper();
		...
		// In real case it should be 0, but it doesn't really matter.
		// Let's check that non-zero is handled well.
		var calories = 1000;
	...
	(:test)
	function success_calculateActiveCalories_zero_at_noon_when_low_calories_male(logger){
		var uiH = new helper();

		var noon = new Time.Gregorian.Info();
		noon.year = 2020;
		noon.hour = 12;
		noon.min = 0;
		...
		// A 41 year old person should have BMR above a 1000 a day.
		// Let's use half of it as it is just noon.
		var calories = 500;

		Test.assertEqual(0, uiH.calculateActiveCalories(noon, profile, calories));

		return true;
	}

	(:test)
	function success_calculateActiveCalories_zero_at_noon_when_low_calories_female(logger){
		var uiH = new helper();
	...
	(:test)
	function success_calculateActiveCalories_at_noon_when_high_calories_male(logger){
		var uiH = new helper();
		...
		// A 41 year old person should have BMR below a 2000 a day.
		// Let's use much more.
		var calories = 3000;

		Test.assert(0 < uiH.calculateActiveCalories(noon, profile, calories));

		return true;
	}

	(:test)
	function success_calculateActiveCalories_at_noon_when_high_calories_female(logger){
		var uiH = new helper();
	...
	(:test)
	function success_calculateActiveCalories_lower_at_evening_than_at_noon_male(logger){
		var uiH = new helper();
		...
		var evening = new Time.Gregorian.Info();
		evening.year = 2020;
		evening.hour = 21;
		evening.min = 0;
		...
		var caloriesNoon = uiH.calculateActiveCalories(noon, profile, calories);

		var caloriesEvening = uiH.calculateActiveCalories(evening, profile, calories);

		Test.assert(caloriesEvening < caloriesNoon);

		return true;
	}

	(:test)
	function success_calculateActiveCalories_lower_at_evening_than_at_noon_female(logger){
		var uiH = new helper();
